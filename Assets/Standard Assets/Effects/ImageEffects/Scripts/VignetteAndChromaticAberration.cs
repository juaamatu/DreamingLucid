    ...
    public class VignetteAndChromaticAberration : PostEffectsBase
    {
        ...
        void OnRenderImage (RenderTexture source, RenderTexture destination)
        {
            ...
            if (doPrepass)
            {
                ...
                Vector3 coord = GetComponent<Camera>().WorldToScreenPoint(player.position);
                coord = new Vector3(coord.x / Screen.width, coord.y / Screen.height);
                //m_VignetteMaterial.SetVector("_PlayerCoordinate", new Vector4(coord.x, coord.y, 0, 0));		// intensity for vignette
