...
public class LensAberrations : MonoBehaviour
{
    ...
    private UnityStandardAssets.ImageEffects.ColorCorrectionCurves m_ColorCorrectionCurves;
    private UnityStandardAssets.ImageEffects.ContrastEnhance m_ContrastEnchance;
    ...
    private void Awake()
    {
        ...
        m_ContrastEnchance = GetComponent<UnityStandardAssets.ImageEffects.ContrastEnhance>();
        m_ColorCorrectionCurves = GetComponent<UnityStandardAssets.ImageEffects.ColorCorrectionCurves>();
    }
    ...
    {
        Vector3 playerScreenPos = m_Camera.WorldToScreenPoint(m_Player.position);
        float x = playerScreenPos.x / Screen.width;
        float y = (m_ContrastEnchance.enabled || m_ColorCorrectionCurves.enabled) ? playerScreenPos.y / Screen.height : 1 - playerScreenPos.y / Screen.height;
        material.SetVector(m_VignetteCenter, new Vector2(x,y));
        distortion.amount = Mathf.Sin(Time.time) * 20;
    }
